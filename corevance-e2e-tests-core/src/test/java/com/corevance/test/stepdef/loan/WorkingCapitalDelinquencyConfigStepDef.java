...
import com.corevance.client.models.DelinquencyBucketTemplateResponse;
import com.corevance.client.models.DelinquencyMinimumPaymentPeriodAndRuleResponse;
import com.corevance.client.models.DelinquencyRangeResponse;
import com.corevance.client.models.MinimumPaymentPeriodAndRule;
import com.corevance.client.models.PostDelinquencyBucketResponse;
import com.corevance.test.data.delinquency.DelinquencyBucketType;
import com.corevance.test.data.delinquency.DelinquencyFrequencyType;
import com.corevance.test.data.delinquency.DelinquencyMinimumPayment;
...
public class WorkingCapitalDelinquencyConfigStepDef extends AbstractStepDef {
    ...
    public void adminCallsDelinquencyTemplate() {
        DelinquencyBucketTemplateResponse template = ok(
                () -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().getBucketTemplate());
    ...
    public void getDelinquencyBucketWithTemplateHasTheFollowingValues() {
        ...
        DelinquencyBucketResponse delinquencyBucketData = ok(
                () -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().getBucket(id, Map.of("template", "true")));
    ...
    public void adminCreatesWCDelinquencyBucketWithValues() {
        ...
        PostDelinquencyBucketResponse ok = ok(
                () -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().createBucket(delinquencyBucketRequest));
    ...
    public void adminModifiesWCDelinquencyBucketWithValues() {
        Long id = TestContext.GLOBAL.get(TestContextKey.DELINQUENCY_BUCKET_ID);
        DelinquencyBucketResponse delinquencyBucketData = ok(
                () -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().getBucket(id));
        DelinquencyBucketRequest delinquencyBucketRequest = new DelinquencyBucketRequest() //
        ...
        ok(() -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().updateBucket(id, delinquencyBucketRequest));
    ...
    public void adminDeletesWCDelinquencyBucketWithValues() {
        Long id = TestContext.GLOBAL.get(TestContextKey.DELINQUENCY_BUCKET_ID);
        ok(() -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().deleteBucket(id));
    }
    ...
    public void checkRetrieveWCDelinquencyBucketNotFoundFailure(Long id) {
        CallFailedRuntimeException exception = fail(() -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().getBucket(id));
    ...
    public void checkCreateWCDelinquencyBucketWithInvalidDataFailure(DelinquencyBucketRequest defaultWCDelinquencyBucketCreateRequest,
        ...
        CallFailedRuntimeException exception = fail(
                () -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().createBucket(defaultWCDelinquencyBucketCreateRequest));
    ...
    public void checkUpdateWCDelinquencyBucketWithInvalidDataFailure(Long id, DelinquencyBucketRequest defaultWCDelinquencyBucketRequest,
            String errorMessage, int errorCode) {
        CallFailedRuntimeException exception = fail(
                () -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().updateBucket(id, defaultWCDelinquencyBucketRequest));
        assertThat(exception.getStatus()).as(ErrorMessageHelper.incorrectExpectedValueInResponse()).isEqualTo(errorCode);
        assertThat(exception.getDeveloperMessage()).contains(errorMessage);
    }

    public void checkDeleteWCDelinquencyBucketDoesntExistFailure(Long id) {
        CallFailedRuntimeException exception = fail(() -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().deleteBucket(id));
    ...
    public void adminFailedToDeleteWCDelinquencyBucketAssignedToLoanProduct() {
        ...
        final CallFailedRuntimeException exception = fail(
                () -> corevanceFeignClient.delinquencyRangeAndBucketsManagement().deleteBucket(id));
