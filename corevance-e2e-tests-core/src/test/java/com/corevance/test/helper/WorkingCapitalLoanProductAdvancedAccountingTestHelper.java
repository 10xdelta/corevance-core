...
import com.corevance.client.models.GetGLAccountsResponse;
...
import com.corevance.client.models.PostGLAccountsRequest;
...
import com.corevance.client.models.WorkingCapitalLoanPaymentChannelToFundSourceMappings;
import com.corevance.client.models.WorkingCapitalLoanProductChargeToGLAccountMapper;
import com.corevance.client.models.WorkingCapitalPostChargeOffReasonToExpenseAccountMappings;
import com.corevance.client.models.WorkingCapitalPostWriteOffReasonToExpenseAccountMappings;
import com.corevance.test.data.ChargeCalculationType;
import com.corevance.test.data.ChargePaymentMode;
import com.corevance.test.data.ChargeTimeType;
import com.corevance.test.data.GLAType;
import com.corevance.test.data.GLAUsage;
...
import com.corevance.test.factory.GLAccountRequestFactory;
...
public final class WorkingCapitalLoanProductAdvancedAccountingTestHelper {
    ...
    public static AdvancedAccountingExpectation prepareAdvancedMappings(final PostWorkingCapitalLoanProductsRequest request,
            final PaymentTypeResolver paymentTypeResolver, final CorevanceFeignClient corevanceFeignClient) {
    ...
    private static AdvancedAccountingExpectation prepareAdvancedMappingsInternal(final Consumer<Long> setFundSourceAccountId,
            ...
            final PaymentTypeResolver paymentTypeResolver, final CorevanceFeignClient corevanceFeignClient) {
        final Long fundSourceAccountId = resolveOrCreateGLAccount(corevanceFeignClient, "WC E2E Mapping Fund Source", "WCE2EFS1",
                GLAType.ASSET.value);
        final Long feeIncomeAccountId = resolveOrCreateGLAccount(corevanceFeignClient, "WC E2E Mapping Fee Income", "WCE2EFI1",
                GLAType.INCOME.value);
        final Long penaltyIncomeAccountId = resolveOrCreateGLAccount(corevanceFeignClient, "WC E2E Mapping Penalty Income", "WCE2EPI1",
                GLAType.INCOME.value);
        final Long writeOffExpenseAccountId = resolveOrCreateGLAccount(corevanceFeignClient, "WC E2E Mapping Write Off Expense", "WCE2EWE1",
        ...
        final Long feeChargeId = createChargeForAdvancedMappings(corevanceFeignClient, false, 2.0);
        final Long penaltyChargeId = createChargeForAdvancedMappings(corevanceFeignClient, true, 1.0);
        final Long chargeOffReasonId = resolveFirstCodeValueId(corevanceFeignClient, "ChargeOffReasons");
        final Long writeOffReasonId = resolveFirstCodeValueId(corevanceFeignClient, "WriteOffReasons");
    ...
    private static AdvancedAccountingExpectation buildExpectation(final Long paymentTypeId, final Long fundSourceAccountId,
            ...
            final Long writeOffExpenseAccountId, final CorevanceFeignClient corevanceFeignClient) {
        final List<GetGLAccountsResponse> glAccounts = ok(
                () -> corevanceFeignClient.generalLedgerAccount().retrieveAllAccountsUniversal(Map.of()));
    ...
    private static Long createChargeForAdvancedMappings(final CorevanceFeignClient corevanceFeignClient, final boolean penalty,
            final double amount) {
        ...
        final PostChargesResponse response = ok(() -> corevanceFeignClient.charges().createCharge(request));
        return response.getResourceId();
    }

    private static Long resolveOrCreateGLAccount(final CorevanceFeignClient corevanceFeignClient, final String name, final String glCode,
            final Integer type) {
        ...
        List<GetGLAccountsResponse> accounts = ok(() -> corevanceFeignClient.generalLedgerAccount().retrieveAllAccountsUniversal(Map.of()));
        ...
        final PostGLAccountsRequest request = GLAccountRequestFactory.defaultGLAccountRequest(name, glCode, type, GLAUsage.DETAIL.value,
                true);
        executeVoid(() -> corevanceFeignClient.generalLedgerAccount().createGLAccount(request, Map.of()));

        accounts = ok(() -> corevanceFeignClient.generalLedgerAccount().retrieveAllAccountsUniversal(Map.of()));
    ...
    private static Long resolveFirstCodeValueId(final CorevanceFeignClient corevanceFeignClient, final String codeName) {
        final List<GetCodeValuesDataResponse> codeValues = ok(
                () -> corevanceFeignClient.codeValues().retrieveAllCodeValuesByCodeName(codeName, Map.of()));
