 ...
 * under the License.
 */
package com.corevance.test.service;
...
import com.corevance.test.data.job.Job;
import com.corevance.test.data.job.JobResolver;
...
public class JobService {
    ...
    private void execute(Long jobId) {
        ...
        executeVoid(() -> corevanceClient.schedulerJob().executeJob(jobId, new ExecuteJobRequest(), queryParams));
    }
    ...
    private JobDetailHistoryDataSwagger getLatestJobRunHistory(Long jobId) {
        GetJobsJobIDJobRunHistoryResponse response = ok(() -> corevanceClient.schedulerJob().retrieveHistory(jobId, 0, 1, "id", "DESC"));
