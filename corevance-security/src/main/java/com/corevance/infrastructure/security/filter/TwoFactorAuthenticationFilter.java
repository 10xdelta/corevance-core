 ...
 * under the License.
 */
package com.corevance.infrastructure.security.filter;
 ...
 * profile is active.
 *
 * This filter validates an access-token provided as a header 'Corevance-Platform-TFA-Token'. If a valid token is
 ...
 * authority regardless of the value of the 'Corevance-Platform-TFA-Token' header.
 */
...
public class TwoFactorAuthenticationFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (authentication != null && authentication.isAuthenticated() && authentication.getPrincipal() instanceof AppUser) {
            ...
            if (!user.hasSpecificPermissionTo(TwoFactorConstants.BYPASS_TWO_FACTOR_PERMISSION)) {
                ...
                // token
                String token = request.getHeader("Corevance-Platform-TFA-Token");
                if (token != null) {
                    TFAccessToken accessToken = twoFactorService.fetchAccessTokenForUser(user, token);
                    // Token is non-existent or invalid
                    if (accessToken == null || !accessToken.isValid()) {
                        response.addHeader("WWW-Authenticate", "Basic realm=\"Corevance Platform API Two Factor\"");
    ...
    private Authentication createUpdatedAuthentication(final Authentication currentAuthentication,
        ...
        } else if (currentAuthentication instanceof CorevanceJwtAuthenticationToken) {
            CorevanceJwtAuthenticationToken corevanceJwtAuthenticationToken = (CorevanceJwtAuthenticationToken) currentAuthentication;
            CorevanceJwtAuthenticationToken updatedAuthentication = new CorevanceJwtAuthenticationToken(
                    corevanceJwtAuthenticationToken.getToken(), updatedAuthorities, (UserDetails) currentAuthentication.getPrincipal());
            return updatedAuthentication;
        } else {
