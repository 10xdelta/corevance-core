...
import com.corevance.infrastructure.security.data.AuthenticatedUserData;
import com.corevance.infrastructure.security.exception.PasswordResetRequiredException;
import com.corevance.infrastructure.security.service.SpringSecurityPlatformSecurityContext;
...
public class AuthenticationApiResource {
    ...
    @Value("${corevance.security.2fa.enabled}")
    ...
    public String authenticate(@Parameter(hidden = true) final String apiRequestBodyAsJson) {
        // TODO COREVANCE-819: sort out Jersey so JSON conversion does not have
        ...
        if (request == null) {
            throw new IllegalArgumentException(
                    "Invalid JSON in BODY (no longer URL param; see COREVANCE-726) of POST to /authentication: " + apiRequestBodyAsJson);
        }
        if (request.username == null || request.password == null) {
            throw new IllegalArgumentException("Username or Password is null in JSON (see COREVANCE-726) of POST to /authentication: "
