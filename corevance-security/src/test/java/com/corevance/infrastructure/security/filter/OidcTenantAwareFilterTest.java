...
import com.corevance.infrastructure.core.config.CorevanceProperties.CorevanceSecurityProperties.CorevanceSecurityOidcFederationProperties.OidcIssuerProperties;
...
class OidcTenantAwareFilterTest {
    ...
    private static final String TENANT_CLAIM = "corevance_tenant";
    ...
    @Mock
    private CorevanceProperties.CorevanceSecurityProperties securityProperties;
    @Mock
    private CorevanceSecurityOidcFederationProperties oidcProps;
    @Mock
    private TenantOidcConfigService tenantOidcConfigService;
    @Mock
    private CorevancePlatformTenant tenant;
    ...
    @BeforeEach
    void setUp() {
        when(corevanceProperties.getSecurity()).thenReturn(securityProperties);
        ...
        filter = new OidcTenantAwareFilter(bearerTokenResolver, tenantDetailsService, corevanceProperties,
    ...
    // -------------------------------------------------------------------------
    // Priority 3: custom claim (corevance_tenant) — legacy / single-realm
    // -------------------------------------------------------------------------
    ...
    void resolvesTenantFromHttpHeader() throws Exception {
        ...
        request.addHeader("Corevance-Platform-TenantId", TENANT_ID);
        when(tenantDetailsService.loadTenantById(TENANT_ID, false)).thenReturn(tenant);
    ...
    void headerTakesPriorityOverQueryParam() throws Exception {
        when(bearerTokenResolver.resolve(request)).thenReturn(null);
        request.addHeader("Corevance-Platform-TenantId", "from-header");
