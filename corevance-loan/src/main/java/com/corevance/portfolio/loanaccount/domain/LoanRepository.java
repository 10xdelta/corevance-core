...
import com.corevance.cob.data.LoanDataForExternalTransfer;
...
public interface LoanRepository extends JpaRepository<Loan, Long>, JpaSpecificationExecutor<Loan> {
    ...
    String FIND_LOAN_DATA_FOR_EXTERNAL_TRANSFER = "select new com.corevance.cob.data.LoanDataForExternalTransfer(loan.id, loan.externalId, loan.loanStatus, loan.loanProduct.id, loan.loanProduct.shortName) from Loan loan where loan.id = :loanId";
    ...
    @Query("SELECT CASE WHEN COUNT(l) > 0 THEN TRUE ELSE FALSE END FROM Loan l WHERE l.id = :loanId and l.loanRepaymentScheduleDetail.loanScheduleType = com.corevance.portfolio.loanaccount.loanschedule.domain.LoanScheduleType.PROGRESSIVE")
    ...
    @Query("SELECT MAX(lt.dateOf) FROM LoanTransaction lt, Loan l WHERE lt.loan=l AND l.loanStatus = com.corevance.portfolio.loanaccount.domain.LoanStatus.ACTIVE")
