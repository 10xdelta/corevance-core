...
import com.corevance.portfolio.loanaccount.data.AmortizationAllocationMappingDTO;
...
public interface LoanAmortizationAllocationMappingRepository
    ...
    @Query("""
                    SELECT new com.corevance.portfolio.loanaccount.data.AmortizationAllocationMappingDTO(
    ...
    @Query("""
                    SELECT COALESCE(SUM(
                        CASE
                            WHEN laam.amortizationType = com.corevance.portfolio.loanaccount.domain.AmortizationType.AM THEN laam.amount
                            WHEN laam.amortizationType = com.corevance.portfolio.loanaccount.domain.AmortizationType.AM_ADJ THEN (0 - laam.amount)
                            ELSE 0
                        END
                    ), 0)
                    FROM LoanAmortizationAllocationMapping laam
    ...
    @Query("""
                    ...
                    FROM LoanAmortizationAllocationMapping laam
                    WHERE laam.baseLoanTransactionId = :baseLoanTransactionId AND laam.loanId = :loanId
                    AND laam.amortizationType = com.corevance.portfolio.loanaccount.domain.AmortizationType.AM
            """)
