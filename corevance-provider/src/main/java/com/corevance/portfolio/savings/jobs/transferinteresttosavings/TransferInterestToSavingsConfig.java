 ...
 * under the License.
 */
package com.corevance.portfolio.savings.jobs.transferinteresttosavings;
...
@Configuration
public class TransferInterestToSavingsConfig {
    ...
    @Bean
    protected Step transferInterestToSavingsStep() {
        return new StepBuilder(JobName.TRANSFER_INTEREST_TO_SAVINGS.name(), jobRepository)
                .tasklet(transferInterestToSavingsTasklet(), transactionManager).build();
    }

    @Bean
    public Job transferInterestToSavingsJob() {
        return new JobBuilder(JobName.TRANSFER_INTEREST_TO_SAVINGS.name(), jobRepository).start(transferInterestToSavingsStep())
