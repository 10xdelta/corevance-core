...
import com.corevance.portfolio.charge.exception.ChargeCannotBeDeletedException;
...
public class ChargeWritePlatformServiceJpaRepositoryImpl implements ChargeWritePlatformService {
    ...
    @Transactional
    @Override
    @CacheEvict(value = "charges", key = "T(com.corevance.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat('ch')")
    public CommandProcessingResult createCharge(final JsonCommand command) {
        try {
            ...
            corevanceEntityAccessUtil.checkConfigurationAndAddProductResrictionsForUserOffice(
                    CorevanceEntityAccessType.OFFICE_ACCESS_TO_CHARGES, charge.getId());
