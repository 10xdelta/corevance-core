...
import com.corevance.organisation.staff.exception.StaffNotFoundException;
import com.corevance.organisation.staff.exception.StaffRoleException;
...
import com.corevance.portfolio.loanaccount.domain.LoanTopupDetails;
...
public class LoanAssemblerImpl implements LoanAssembler {
    ...
    // COREVANCE-2665: recalculate the schedule from the loan's own state (same as approval/reschedule/holidays).
    ...
    private void copyAdvancedPaymentRulesIfApplicable(String transactionProcessingStrategyCode, LoanProduct loanProduct,
            Loan loanApplication) {
        if (transactionProcessingStrategyCode.equals(PhasedSettlementScheduleProcessor.ADVANCED_PAYMENT_ALLOCATION_STRATEGY)) {
    ...
    public Map<String, Object> updateFrom(JsonCommand command, Loan loan) {
        ...
        if (changes.containsKey("recalculateLoanSchedule")) {
            ...
            // COREVANCE-2665: recalculate from the loan's own (already-modified) state, exactly like every other
