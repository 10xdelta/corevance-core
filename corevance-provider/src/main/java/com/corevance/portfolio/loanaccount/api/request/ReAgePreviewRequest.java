 ...
 * under the License.
 */
package com.corevance.portfolio.loanaccount.api.request;
...
public class ReAgePreviewRequest implements Serializable {
    ...
    @NotNull(message = "{com.corevance.reage.frequency-number.not-blank}")
    @Min(value = 1, message = "{com.corevance.reage.frequency-number.min}")
    private Integer frequencyNumber;

    @QueryParam("frequencyType")
    @Parameter(description = "The frequency type (DAYS, WEEKS, MONTHS, YEARS)", required = true)
    @NotBlank(message = "{com.corevance.reage.frequency-type.not-blank}")
    @EnumValue(enumClass = PeriodFrequencyType.class, message = "{com.corevance.frequency-type.invalid}")
    private String frequencyType;

    @QueryParam("startDate")
    @Parameter(description = "The start date for the re-aging schedule", required = true)
    @NotBlank(message = "{com.corevance.reage.start-date.not-blank}")
    private String startDate;

    @QueryParam("numberOfInstallments")
    @Parameter(description = "The number of installments for the re-aged loan", required = true)
    @NotNull(message = "{com.corevance.reage.number-of-installments.not-blank}")
    @Min(value = 1, message = "{com.corevance.reage.number-of-installments.min}")
