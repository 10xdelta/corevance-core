 ...
 * under the License.
 */
package com.corevance.portfolio.loanaccount.jobs.recalculateinterestforloan;
...
import com.corevance.portfolio.loanaccount.service.RecalculateInterestPoster;
    ...
    private void recalculateInterest(List<Long> loanIds, int threadPoolSize) {
        ...
        List<Future<Void>> responses = ListUtils.partition(loanIds, actualBatchSize).stream().filter(subList -> !subList.isEmpty())
                .map(subList -> {
                    ...
                    recalculateInterestPoster.setCorevanceContext(ThreadLocalContextUtil.getContext());
