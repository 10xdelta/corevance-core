...
public class WebHookProcessor implements HookProcessor {
    ...
    public void process(final Hook hook, final String payload, final String entityName, final String actionName,
            final CorevanceContext context) {
    ...
    private void sendRequest(final String url, final String contentType, final String payload, final String entityName,
            final String actionName, final CorevanceContext context) {

        final String corevanceEndpointUrl = System.getProperty("baseUrl");
        ...
        if (contentType.equalsIgnoreCase("json") || contentType.contains("json")) {
            ...
            service.sendJsonRequest(entityName, actionName, context.getTenantContext().getTenantIdentifier(), corevanceEndpointUrl, json)
                    .enqueue(callback);
        } else {
            Map<String, String> map = new HashMap<>();
            map = new Gson().fromJson(payload, map.getClass());
            service.sendFormRequest(entityName, actionName, context.getTenantContext().getTenantIdentifier(), corevanceEndpointUrl, map)
                    .enqueue(callback);
        }
    }
}
