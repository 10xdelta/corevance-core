...
import static com.corevance.infrastructure.jobs.service.aggregationjob.JournalEntryAggregationJobConstant.JOB_TRACKING_STEP_NAME;
...
@Configuration
@ConditionalOnProperty(value = "corevance.job.journal-entry-aggregation.enabled", havingValue = "true")
    ...
    public Step journalEntryAggregationSummaryStep() {
        return new StepBuilder(JOB_SUMMARY_STEP_NAME, jobRepository)
                .<JournalEntryAggregationSummaryData, JournalEntryAggregationSummaryData>chunk(
                        corevanceProperties.getJob().getJournalEntryAggregation().getChunkSize(), transactionManager)
