 ...
 * under the License.
 */
package com.corevance.infrastructure.core.service.migration;
...
public class TenantDatabaseUpgradeService implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        if (notLiquibaseOnlyMode()) {
            if (databaseStateVerifier.isLiquibaseDisabled() || !corevanceProperties.getMode().isWriteEnabled()) {
                log.warn("Liquibase is disabled. Not upgrading any database.");
                if (!corevanceProperties.getMode().isWriteEnabled()) {
                    log.warn("Liquibase is disabled because the current instance is configured as a non-write Corevance instance");
                }
                return;
            }
        }
    ...
    private boolean notLiquibaseOnlyMode() {
        ...
        return !activeProfiles.contains(CorevanceProfiles.LIQUIBASE_ONLY);
    }

    private void upgradeTenantStore() throws LiquibaseException {
        log.info("Upgrading tenant store DB at {}:{}", corevanceProperties.getTenant().getHost(), corevanceProperties.getTenant().getPort());
    ...
    private void logTenantStoreDetails() {
        CorevanceProperties.CorevanceTenantProperties tenant = corevanceProperties.getTenant();
        log.info("- corevance.tenant.username: {}", tenant.getUsername());
        log.info("- corevance.tenant.password: ****");
        log.info("- corevance.tenant.parameters: {}", tenant.getParameters());
        log.info("- corevance.tenant.timezone: {}", tenant.getTimezone());
        log.info("- corevance.tenant.description: {}", tenant.getDescription());
        log.info("- corevance.tenant.identifier: {}", tenant.getIdentifier());
        log.info("- corevance.tenant.name: {}", tenant.getName());
        ...
        if (isNotBlank(readOnlyUsername)) {
            log.info("- corevance.tenant.readonly.username: {}", readOnlyUsername);
            log.info("- corevance.tenant.readonly.password: {}", isNotBlank(tenant.getReadOnlyPassword()) ? "****" : "");
            log.info("- corevance.tenant.readonly.parameters: {}", tenant.getReadOnlyParameters());
            log.info("- corevance.tenant.readonly.name: {}", tenant.getReadOnlyName());
        }
    ...
    private void upgradeIndividualTenants() {
        ...
        List<CorevancePlatformTenant> tenants = tenantDetailsService.findAllTenants();
        ...
        if (isNotEmpty(tenants)) {
            for (CorevancePlatformTenant tenant : tenants) {
    ...
    private ThreadPoolTaskExecutor createTenantUpgradeThreadPoolTaskExecutor() {
        ...
        threadPoolTaskExecutor.setCorePoolSize(corevanceProperties.getTaskExecutor().getTenantUpgradeTaskExecutorCorePoolSize());
        threadPoolTaskExecutor.setMaxPoolSize(corevanceProperties.getTaskExecutor().getTenantUpgradeTaskExecutorMaxPoolSize());
        threadPoolTaskExecutor.setQueueCapacity(corevanceProperties.getTaskExecutor().getTenantUpgradeTaskExecutorQueueCapacity());
     ...
     */
    private void upgradeIndividualTenant(CorevancePlatformTenant tenant) throws LiquibaseException {
        try {
    ...
    private void applyInitialLiquibase(DataSource dataSource, ExtendedSpringLiquibase liquibase, String id,
        ...
        if (databaseStateVerifier.isFlywayPresent(dataSource)) {
            if (isUpgradableFn.apply(dataSource)) {
                ...
                throw new SchemaUpgradeNeededException("Make sure to upgrade to Corevance 1.6 first and then to a newer version");
            }
