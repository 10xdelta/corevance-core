...
package com.corevance.infrastructure.core.config.cache;

import static com.corevance.infrastructure.event.external.repository.ExternalEventConfigurationRepository.EXTERNAL_EVENT_CONFIGURATION_CACHE_NAME;
...
public class CacheConfig {
    ...
    private CacheManager getInternalEhCacheManager() {
        ...
        Duration defaultTimeToLive = corevanceProperties.getCache().getDefaultTemplate().getTtl();
        Integer defaultMaxEntries = corevanceProperties.getCache().getDefaultTemplate().getMaximumEntries();
        ...
        Set<String> incorrectConfigurations = new HashSet<>(corevanceProperties.getCache().getCustomTemplates().keySet());
    ...
    private javax.cache.configuration.Configuration<Object, Object> generateCustomCacheConfiguration(String cacheIdentifier,
        ...
        if (corevanceProperties.getCache().getCustomTemplates().containsKey(cacheIdentifier)) {
            Duration timeToLiveExpiration = Objects.requireNonNullElse(
                    corevanceProperties.getCache().getCustomTemplates().get(cacheIdentifier).getTtl(), defaultTimeToLive);
            Integer maxEntries = Objects.requireNonNullElse(
                    corevanceProperties.getCache().getCustomTemplates().get(cacheIdentifier).getMaximumEntries(), defaultMaxEntries);
