...
public class KafkaExternalEventTopicConfig {
    ...
    public KafkaAdmin admin() {
        Map<String, Object> props = new HashMap<>(
                corevanceProperties.getEvents().getExternal().getProducer().getKafka().getAdmin().getExtraPropertiesMap());
        props.put(BOOTSTRAP_SERVERS_CONFIG, corevanceProperties.getEvents().getExternal().getProducer().getKafka().getBootstrapServers());
        return new KafkaAdmin(props);
    }

    @Bean
    public NewTopic externalEventsTopic() {
        CorevanceProperties.KafkaTopicProperties topicProperties = corevanceProperties.getEvents().getExternal().getProducer().getKafka()
                .getTopic();
