...
import com.corevance.infrastructure.core.config.CorevanceProperties.CorevanceExternalEventsProducerJmsProperties;
...
@Configuration
@ConditionalOnProperty(value = "corevance.events.external.producer.jms.enabled", havingValue = "true")
public class ExternalEventJMSConfiguration {
    ...
    public CachingConnectionFactory connectionFactory() {
        CorevanceExternalEventsProducerJmsProperties jmsProps = corevanceProperties.getEvents().getExternal().getProducer().getJms();
    ...
    public ActiveMQTopic activeMqTopic() {
        return new ActiveMQTopic(corevanceProperties.getEvents().getExternal().getProducer().getJms().getEventTopicName());
    }

    @Conditional(EnableExternalEventQueueCondition.class)
    @Bean(name = "externalEventDestination")
    public ActiveMQQueue activeMqQueue() {
        return new ActiveMQQueue(corevanceProperties.getEvents().getExternal().getProducer().getJms().getEventQueueName());
    }
    ...
    public ThreadPoolTaskExecutor externalEventJmsProducerExecutor() {
        ...
        threadPoolTaskExecutor.setCorePoolSize(
                corevanceProperties.getEvents().getExternal().getProducer().getJms().getThreadPoolTaskExecutorCorePoolSize());
        threadPoolTaskExecutor
                .setMaxPoolSize(corevanceProperties.getEvents().getExternal().getProducer().getJms().getThreadPoolTaskExecutorMaxPoolSize());
