...
Feature: SQL Builder
  ...
  Scenario Outline: Verify that SQL builder has expected template and arguments
    ...
    Examples:
      ...
      |            |                       |            |                       |           |           |           |           |                                                  | SQLBuilder{}                                                                                       |
      | name =     | Michael               | hobby LIKE | Mifos/Apache Corevance | age <     | 123       |           |           | WHERE  name = ? AND hobby LIKE ? AND age < ?     | SQLBuilder{WHERE name = ['Michael'] AND hobby LIKE ['Mifos/Apache Corevance'] AND age < [123]}      |
      | ref =      | NULL                  |            |                       |           |           |           |           | WHERE  ref = ?                                   | SQLBuilder{WHERE ref = [null]}                                                                     |
      | hobby LIKE | Mifos/Apache Corevance | hobby like | Mifos/Apache Corevance |           |           |           |           | WHERE  hobby LIKE ? AND hobby like ?             |                                                                                                    |
