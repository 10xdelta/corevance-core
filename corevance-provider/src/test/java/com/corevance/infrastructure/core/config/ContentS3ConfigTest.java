...
class ContentS3ConfigTest {
    ...
    @Mock
    private CorevanceProperties.CorevanceContentProperties contentProperties;

    @Mock
    private CorevanceProperties.CorevanceContentS3Properties s3Properties;

    @BeforeEach
    void setUp() {
        when(corevanceProperties.getContent()).thenReturn(contentProperties);
        when(contentProperties.getS3()).thenReturn(s3Properties);
    }

    @AfterEach
    void tearDown() {
        reset(corevanceProperties);
    ...
    void testContentS3Client_WithCredentialsAndNoEndpoint() {
        ...
        S3Client s3Client = contentS3Config.contentS3Client(corevanceProperties);
