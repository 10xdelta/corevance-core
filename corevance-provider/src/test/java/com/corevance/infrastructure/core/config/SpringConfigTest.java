...
class SpringConfigTest {
    ...
    void smartDefaultsUseCpuAwarePoolSizing() {
        ...
        ThreadPoolTaskExecutor executor = config.corevanceEventExecutor(builder, -1, -1, -1);
    ...
    void userPropertiesOverrideSmartDefaults() {
        ...
        ThreadPoolTaskExecutor executor = config.corevanceEventExecutor(builder, 16, 40, 200);
    ...
    void absoluteSafetyAdjustsMaxWhenUserSetsHighCoreButLowMax() {
        ...
        ThreadPoolTaskExecutor executor = config.corevanceEventExecutor(builder, 50, 40, -1);
    ...
    void absoluteSafetyAdjustsMaxWhenSmartDefaultTooSmallForUserCore() {
        ...
        ThreadPoolTaskExecutor executor = config.corevanceEventExecutor(builder, smartMax + 10, -1, -1);
    ...
    void boundedPoolPreventsThreadExplosionUnderHighLoad() throws Exception {
        ...
        executor.setThreadNamePrefix("CorevanceEvent-");
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy());
        executor.initialize();
        ...
        try {
            ...
            long poolThreadCount = threadNames.stream().filter(name -> name.startsWith("CorevanceEvent-")).count();
            ...
            boolean mainThreadHelped = threadNames.stream().anyMatch(name -> !name.startsWith("CorevanceEvent-"));
    ...
    @DisplayName("Event executor is active in all Corevance modes")
    @CsvSource({ "corevance.mode.read-enabled=true", "corevance.mode.write-enabled=true", "corevance.mode.batch-worker-enabled=true",
            "corevance.mode.batch-manager-enabled=true" })
    void verifyExecutorIsActiveInMode(String modeProperty) {
        contextRunner.withPropertyValues(modeProperty).run(context -> {
            assertThat(context).hasBean("corevanceEventExecutor");
            ThreadPoolTaskExecutor executor = context.getBean("corevanceEventExecutor", ThreadPoolTaskExecutor.class);
            assertThat(executor.getThreadNamePrefix()).isEqualTo("CorevanceEvent-");
        });
    }
}
