...
class CorevanceExternalEventConfigConditionTest {
    ...
    @InjectMocks
    private CorevanceExternalEventConfigCondition underTest = new CorevanceExternalEventConfigCondition();
    ...
    public void testMatchesShouldReturnFalseWhenPartitionSizeIsNotBiggerThanLimit() {
        // given
        environment.withProperty("corevance.events.external.partition-size", "25000");
        // when
    ...
    public void testMatchesShouldReturnTrueWhenPartitionSizeIsBiggerThanLimit() {
        // given
        environment.withProperty("corevance.events.external.partition-size", "25001");
        // when
        boolean result = underTest.matches(conditionContext, metadata);
        // then
    ...
    public void testMatchesShouldReturnTrueWhenPartitionSizeIsNotPositive() {
        // given
        environment.withProperty("corevance.events.external.partition-size", "0");
        // when
        boolean result = underTest.matches(conditionContext, metadata);
        // then
        assertThat(result).isTrue();
