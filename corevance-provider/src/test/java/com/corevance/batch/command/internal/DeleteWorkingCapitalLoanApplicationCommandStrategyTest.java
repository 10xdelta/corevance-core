...
/**
 * Test class for {@link DeleteWorkingCapitalLoanApplicationCommandStrategy}.
 */
public class DeleteWorkingCapitalLoanApplicationCommandStrategyTest {

    /**
     * Test {@link DeleteWorkingCapitalLoanApplicationCommandStrategy#execute} happy path scenario.
     */
    @Test
    public void testExecuteSuccessScenario() {
        ...
        given(testContext.workingCapitalLoanApiResource.deleteLoanApplication(eq(loanId))).willReturn(processingResult);
        ...
        verify(testContext.workingCapitalLoanApiResource).deleteLoanApplication(eq(loanId));
        verify(testContext.toApiJsonSerializer).serialize(processingResult);
    }

    /**
     * Creates and returns a batch request for {@code DELETE working-capital-loans/{loanId}}.
     *
     * @param loanId
     *            the loan id
     * @return BatchRequest
    ...
    private BatchRequest getBatchRequest(final Long loanId) {
        ...
        br.setRelativeUrl("v1/working-capital-loans/" + loanId);
    ...
    private static class TestContext {
        ...
        private final DeleteWorkingCapitalLoanApplicationCommandStrategy subjectToTest;

        TestContext() {
            MockitoAnnotations.openMocks(this);
            subjectToTest = new DeleteWorkingCapitalLoanApplicationCommandStrategy(workingCapitalLoanApiResource, toApiJsonSerializer);
        }
    }
}
