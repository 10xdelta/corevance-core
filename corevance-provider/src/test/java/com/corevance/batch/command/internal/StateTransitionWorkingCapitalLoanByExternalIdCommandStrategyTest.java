...
/**
 * Test class for {@link StateTransitionWorkingCapitalLoanByExternalIdCommandStrategy}.
 */
public class StateTransitionWorkingCapitalLoanByExternalIdCommandStrategyTest {

    /**
     * Test {@link StateTransitionWorkingCapitalLoanByExternalIdCommandStrategy#execute} happy path scenario.
     */
    @Test
    public void testExecuteSuccessScenario() {
        ...
        given(testContext.workingCapitalLoanApiResource.stateTransitionByExternalId(eq(loanExternalId), eq(command), eq(request.getBody())))
        ...
        verify(testContext.workingCapitalLoanApiResource).stateTransitionByExternalId(eq(loanExternalId), eq(command),
                eq(request.getBody()));
     ...
     * working-capital-loans/external-id/{loanExternalId}?command={command}}.
     *
    ...
    private static class TestContext {
        ...
        private final StateTransitionWorkingCapitalLoanByExternalIdCommandStrategy subjectToTest;

        TestContext() {
            MockitoAnnotations.openMocks(this);
            subjectToTest = new StateTransitionWorkingCapitalLoanByExternalIdCommandStrategy(workingCapitalLoanApiResource,
                    toApiJsonSerializer);
        }
    }
}
