...
Corevance is capable of generating and raising events for external consumers in a reliable way. This section is going to describe all the details on that front with examples.
...
Whether or not an event must be sent to downstream message channels for a particular Corevance instance is configurable through the UI and API.
...
The whole framework is built upon an existing infrastructure in Corevance; the Business Events.

As a quick recap, Business Events are Corevance events that can be raised at any place in a business operation using the `BusinessEventNotifierService`. Callbacks can be registered when a certain type of Business Event is raised and other business operations can be done. For example when a Loan gets disbursed, there's an interested party doing the Loan Arrears Aging recalculation using the Business Event communication.
...
The database plays a crucial part in the framework since to ensure transactionality, - without doing proper transaction synchronization between different message channels and the database - the framework is going to save all the raised events into the same relational database that Corevance is using.
...
Not null.
|`com.corevance.avro.loan.v1.LoanAccountDataV1`
...
=== Avro schemas
For serializing events, Corevance is using Apache Avro. There are 2 reasons for that:

* More compact storage since Avro is a binary format
* The Avro schemas are published with Corevance as a separate JAR so event consumers can directly map the events into POJOs

There are 3 different levels of Avro schemas used in Corevance for the Reliable event framework which are described below.
...
[source,json]
----
include::{rootdir}/corevance-avro-schemas/src/main/avro/office/v1/OfficeDataV1.avsc[]
----
====
...
[source,json]
----
include::{rootdir}/corevance-avro-schemas/src/main/avro/MessageV1.avsc[]
----
...
NOTE: The ByteBuffer is a stateful container and needs to be handled carefully. Therefore Corevance has a built-in ByteBuffer to byte array converter; `ByteBufferConverter`.
...
This job is a Corevance job, scheduled to run for every minute and will pick a number of events in order. Those events will be put onto the downstream message channel in the same order as they were raised.

=== Purging events
The events database table is going to grow continuously. That's why Corevance has a purging functionality in place that's gonna delete old and already sent events.

It's implemented as a Corevance job and is disabled by default. It's called TBD.
...
* the `corevance.events.external.enabled` property
* the `COREVANCE_EXTERNAL_EVENTS_ENABLED` environment variable
...
=== Raising events
Raising events is really easy. An instance of a BusinessEvent interface is needed, that's going to be the event. There are plenty of them available already in the Corevance codebase.
...
----
{
    ...
    "dataschema": "com.corevance.avro.loan.v1.LoanAccountDataV1",
    "data": "..."
}
----

NOTE: The source attribute refers to an ID that's identifying the producer service. Corevance will regenerate this ID upon each application startup.
...
[source,java]
----
include::{rootdir}/corevance-core/src/main/java/com/corevance/infrastructure/event/business/domain/BusinessEvent.java[lines=21..]
----
...
[source,java]
----
include::{rootdir}/corevance-core/src/main/java/com/corevance/infrastructure/event/external/service/serialization/serializer/BusinessEventSerializer.java[lines=25..]
----
...
New Avro schemas can be easily created. Just create a new Avro schema file in the `corevance-avro-schemas` project under the respective bounded context folder, and it will be picked up automatically by the code generator.
...
[source,json]
----
include::{rootdir}/corevance-avro-schemas/src/main/resources/avro-templates/bigdecimal.avsc[]
----

It's a 20 precision and 8 scale BigDecimal.

Obviously it's quite challenging to copy-paste this snippet to every single BigDecimal field, so there's a customization in place for Corevance.
...
|`corevance.events.external.enabled`
|`COREVANCE_EXTERNAL_EVENTS_ENABLED`
|`false`
