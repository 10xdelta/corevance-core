...
To achieve idempotency in HTTP requests, you can use the HTTP header from `corevance.idempotency-key-header-name` configuration variables (default `Idemptency-Key`). This header is a unique identifier for the request. If you send the same request twice, the second request will be ignored and the response from the first request will be returned.

== Idempotency in Batch requests

In batch requests, you can set the idempotency key for every batch item, in the batch item `header` fields. The header key is from `corevance.idempotency-key-header-name` configuration variables (default `Idemptency-Key`).
