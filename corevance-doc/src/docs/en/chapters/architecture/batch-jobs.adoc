...
== Batch jobs in Corevance
=== Types of jobs
The jobs in Corevance can be divided into 2 categories:

* Normal batch jobs
* Partitionable batch jobs

Most of the jobs are normal batch jobs with limited scalability because Corevance is still passing through the evolution on making most of them capable to process a high-volume of data.
...
When Corevance is started, the database dependent schema SQL script will be picked up according to the datasource configurations.
...
For remote partitioning, the following message channels are supported by Corevance:
...
The business steps are configurable for certain jobs. The reason for that is because we want to allow the possibility for Corevance users to configure their very own business logic for generic jobs, like the Loan Close Of Business job where we want to do a formal "closing" of the loans at the end of the day.
...
[source,text]
----
GET /corevance-provider/api/v1/jobs/{jobName}/steps?tenantIdentifier={tenantId}
HTTP 200
...
[source,text]
----
PUT /corevance-provider/api/v1/jobs/{jobName}/steps?tenantIdentifier={tenantId}
...
[source,text]
----
POST /corevance-provider/api/v1/jobs/LOAN_COB/inline?tenantIdentifier={tenantId}
...
|LOAN_CLOSE_OF_BUSINESS
|corevance.job.loan-cob-enabled
|COREVANCE_JOB_LOAN_COB_ENABLED
|true
