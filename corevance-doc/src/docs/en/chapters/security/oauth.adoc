...
Corevance has basic OAuth support based on Spring Boot Security.
...
[source,bash]
----
COREVANCE_SECURITY_BASICAUTH_ENABLED=false
COREVANCE_SECURITY_OAUTH_ENABLED=true
COREVANCE_SERVER_OAUTH_RESOURCE_URL=http://localhost:9000/realms/corevance
----
...
[source,bash]
----
java -Dcorevance.security.basicauth.enabled=false -Dcorevance.security.oauth2.enabled=true -jar corevance-provider.jar
----
...
. Enter name `corevance` for the realm name
...
----
curl --request POST \
  "$COREVANCE_SERVER_OAUTH_RESOURCE_URL/protocol/openid-connect/token" \
...
----
curl --insecure \
  'https://localhost:8443/corevance-provider/api/v1/offices' \
  --header 'Corevance-Platform-TenantId: default' \
  --header 'Authorization: bearer <enter the value of the access_token field>'
----

NOTE: See also https://corevance.apache.org/docs/legacy/#authentication_oauth
