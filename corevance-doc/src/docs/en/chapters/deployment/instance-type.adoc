= Corevance Instance types

In cases where Corevance has to deal with high load, it can cause a performance problem for a single Corevance instance.
To overcome this problem, Corevance instances can be started in different instance types for better scalability and performance in a multi-instance environment:
...
If Corevance is running as a single instance, then all of the 3 instance types should be enabled. In this case, there is no need to worry about the configuration, because this is the default behavior.
...
A common solution to dealing with the high load is to deploy 1 write and 1 batch instances and deploy multiple read instances with read replicas of the Corevance database.
...
Also a common scenario when Close of Business jobs are running and Corevance has to deal with a high amount of processes.
(In a future release) Corevance (will be) is able to run this CoB jobs in batches.
...
----
    ...
    +----+----+      +----+----+     +----+----+       +----+----+         +----+----+
         ...
         +----------------+---------------+-----------------+-------------------+
                                  ...
                                  +----------------+
----
These solutions can be mixed with each other, based on the load of the Corevance deployment.

== Configuring instance type via environment variables

The Corevance instance type is configurable via environment variables for the following 3 values:

.Environment variables
|===
^| Instance type ^| Environment variable

| Read instance | COREVANCE_READ_MODE_ENABLED
| Write instance | COREVANCE_WRITE_MODE_ENABLED
| Batch instance | COREVANCE_BATCH_MODE_ENABLED
|===

The environment variable values are booleans (true/false). The Corevance instance can be configured in any combination of these instance types, although if all 3 configurations are false, startup will fail. The default value for all 3 values is true.

The configured Corevance instance types are easily accessible via a single Spring bean, named `CorevanceProperties.CorevanceModeProperties` that has 4 methods: `isReadMode()`, `isWriteMode()`, `isBatchMode()`, `isReadOnlyMode()`
...
A Corevance instance is ONLY able to serve read API calls when it’s configured as a read or write instance. In batch instance mode, it won’t serve read API calls.
...
A Corevance instance is ONLY able to serve write API calls when it’s configured as a write instance. In read or batch instance mode, it won't serve write API calls.
...
A Corevance instance is ONLY able to serve batch API calls when it’s configured as a batch instance. In read or write instance mode, it won’t serve batch API calls.
...
Batch jobs are scheduled only if the Corevance instance running as a batch instance


== Read-only instance type restrictions

If the read mode is enabled, but the write mode and batch mode are disabled, Corevance instance runs in read-only mode.

=== Events are disabled for read-only instances

When a Corevance instance is running in read-only mode, all event receiving/sending will be disabled.
...
If the batch mode is enabled, but the read mode and write mode are disabled, Corevance instance runs in batch-only mode.

=== Receiving events is disabled for batch-only instances

When a Corevance instance is running as batch, event receiving will be disabled while sending events will be still possible since the batch jobs are potentially generating business events.
