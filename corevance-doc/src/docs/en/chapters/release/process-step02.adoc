...
----
project = COREVANCE and fixVersion = {revnumber} and status not in ( Resolved, Done, Accepted, Closed )
----

You can also run the following query to make sure that the issues fixed for the to-be-released version look accurate:
[source,text,subs="attributes+,+macros"]
----
project = COREVANCE and fixVersion = {revnumber}
----
...
----
./gradlew corevanceReleaseStep2 -Pcorevance.release.version={revnumber}
----
