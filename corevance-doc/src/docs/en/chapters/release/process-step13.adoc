...
As discussed in https://issues.apache.org/jira/browse/COREVANCE-1154[COREVANCE-1154], now that everything is final, please do the following to remove the release branch (and just keep the tag), and make sure that everything on the release tag is merged to develop and that e.g. `git describe` works:
...
[source,bash,subs="attributes+,+macros"]
----
./gradlew corevanceReleaseStep13 -Pcorevance.release.version={revnumber}
----
