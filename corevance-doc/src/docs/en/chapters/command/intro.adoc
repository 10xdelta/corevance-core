...
There has been already some preparatory work done concerning type safety, but until now we avoided dealing with the real source of this issue. Corevance's architectures devises read from write requests (https://martinfowler.com/bliki/CQRS.html[CQRS]) for improved scalability.
...
[source,java]
----
include::{rootdir}/corevance-core/src/main/java/com/corevance/commands/service/SynchronousCommandProcessingService.java[lines=69..]
----
