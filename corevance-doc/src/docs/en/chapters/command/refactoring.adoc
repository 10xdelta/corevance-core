...
[source,java]
----
include::{rootdir}/corevance-command-test/src/main/java/com/corevance/command/test/sample/data/DummyRequest.java[lines=30..]
----

=== Transformation between types

If you need to transform/convert two classes then then use https://mapstruct.org/[MapStruct] whenever possible. Place the link:++https://mapstruct.org/[MapStruct]++[MapStruct] interfaces under a sibling package `mapping` next to the `data` package (e.g. `com.corevance.xxx.data` and `com.corevance.xxx.mapping`).

=== Package structure (`com.corevance.xxx.*`)
...
In general avoid too many nesting levels. Ideally we would only have one additional underneath the base package (`com.corevance.*`) where in turn only the above package patterns are used.

== REST API

=== Read Requests

The *read requests* (HTTP `*GET*`) usually only require to refactor (if at all) the business logic services in case they don't return proper Java POJOs. More often than not service functions have a return type of `com.corevance.infrastructure.core.data.CommandProcessingResult`. This type belongs to the legacy command processing mechanics and basically is just a thin wrapper around a JSON object that is stored in a string variable.
...
[source,java]
----
include::{rootdir}/corevance-core/src/main/java/com/corevance/infrastructure/businessdate/api/BusinessDateApiResource.java[lines=48..49;54..60;85..86]
----
...
Very often you'll see the use of `PlatformSecurityContext` (which is a wrapper around Spring Security's `org.springframework.security.core.context.SecurityContextHolder`) to validate read permissions. Avoid this please! Fortunately, the fix is relatively easy. We just need to introduce an "Ant matcher" configuration in the Spring web security configuration (`com.corevance.infrastructure.core.config.SecurityConfig`). The change should look something like this:

.Security Configuration (best practice)
[source,java]
----
include::{rootdir}/corevance-provider/src/main/java/com/corevance/infrastructure/core/config/SecurityConfig.java[lines=82..83;138..142;489]
----
...
Add your translation keys to `corevance-validation/src/main/resources/ValidationMessages.properties`
