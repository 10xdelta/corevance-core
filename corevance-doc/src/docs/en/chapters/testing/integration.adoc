...
Integration tests in Apache Corevance validate the complete API layer and business logic by making HTTP calls to a running Corevance instance. These tests ensure that different components work together correctly and that the API behaves as expected.

== Overview

=== Architecture

* *Location*: `integration-tests/src/test/java/com/corevance/integrationtests`
...
* *Client Library*: Uses Corevance client library for type-safe API interactions
* *Prerequisites*: Running Apache Corevance instance (default: https://localhost:8443)

=== Key Characteristics

* Tests run against a live Corevance instance
...
Integration tests run corevance instance from cargo plugin by default.
...
Create a new test class in `integration-tests/src/test/java/com/corevance/integrationtests/`:
...
# Check Corevance logs (logs go to console/stdout)
# If running in background with output redirection:
# tail -f build/bootRun.log

# Restart Corevance if needed
pkill -f bootRun
./gradlew bootRun &
----
...
----
# Reset database
mysql -u root -pmysql -e "DROP DATABASE corevance_default;"
mysql -u root -pmysql -e "DROP DATABASE corevance_tenants;"
...
mysql -u root -pmysql corevance_default -e "SHOW PROCESSLIST;"

# Kill long-running queries
mysql -u root -pmysql corevance_default -e "KILL <process_id>;"
----
...
* Reuse Corevance instance across test runs
