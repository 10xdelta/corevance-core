...
Apache Corevance's E2E test suite provides comprehensive coverage of business functionality using Cucumber BDD (Behavior-Driven Development) framework. These tests serve as both functional validation and living documentation of the system's capabilities.

== Overview

=== Architecture

* *corevance-e2e-tests-runner*: Contains all Cucumber feature files and test scenarios
* *corevance-e2e-tests-core*: Contains step definitions, test utilities, and supporting code
* *Framework*: Cucumber with Java, using Gherkin syntax for readable test specifications
* *Prerequisites*: Running Apache Corevance instance (typically on port 8443 with HTTPS)

=== Test Organization

* Feature files located in: `corevance-e2e-tests-runner/src/test/resources/features/`
* Step definitions in: `corevance-e2e-tests-core/src/test/java/com/corevance/test/stepdef/`
...
* *Java 21*: Apache Corevance requires Java 21 (Azul Zulu JDK recommended)
...
./gradlew createDB -PdbName=corevance_tenants
./gradlew createDB -PdbName=corevance_default
----
...
E2E tests connect to the running Corevance instance. Default configuration in `corevance-e2e-tests-core/src/test/resources/corevance-test-application.properties`:

[source,properties]
----
# Connection details to running backend
corevance-test.api.base-url=${BASE_URL:https://localhost:8443}
corevance-test.api.username=${TEST_USERNAME:mifos}
corevance-test.api.password=${TEST_PASSWORD:password}
corevance-test.api.tenant-id=${TEST_TENANT_ID:default}
----
...
IMPORTANT: Many E2E tests require pre-configured test data (loan products, charges, configurations). This initialization is controlled by the `corevance-test.initialization.enabled` property.

*Default Behavior*:

* By default, initialization is *DISABLED* (`corevance-test.initialization.enabled=false`)
...
Edit `corevance-e2e-tests-core/src/test/resources/corevance-test-application.properties`:
[source,properties]
----
corevance-test.initialization.enabled=true
----
...
* Business Date is *DISABLED* by default in fresh Corevance installations
...
[source,bash]
----
mysql -u root -pmysql corevance_default -e \
  "UPDATE c_configuration SET enabled = 1 WHERE name = 'enable-business-date';"
----

Method 2 - Via API (After Corevance is Running):
[source,bash]
----
curl -k -X PUT https://localhost:8443/corevance-provider/api/v1/configurations/name/enable-business-date \
...
NOTE: The `Corevance-Platform-TenantId` header is required. Without it, the request can fail with HTTP 400 because tenant context is missing.
...
==== Step 1: Start Corevance

[source,bash]
----
# Start Corevance with test profile enabled for E2E
./gradlew bootRun -Dspring.profiles.active=test
----

IMPORTANT: When running E2E tests that hit endpoints/APIs backed by beans annotated with `@Profile(CorevanceProfiles.TEST)`, the provider startup must include `-Dspring.profiles.active=test`. Without this, test-profile-only components are not loaded.

Wait for Corevance to be fully started. You can verify by checking:
[source,bash]
----
...
[source]
----
corevance-e2e-tests-runner/build/reports/allure-report/index.html
----
...
ACTIVEMQ_TOPIC_NAME=corevance-events \
...
1. *Create Feature File*: Add new `.feature` file in `corevance-e2e-tests-runner/src/test/resources/features/`
...
3. *Implement Step Definitions*: Add corresponding step definitions in `corevance-e2e-tests-core/src/test/java/com/corevance/test/stepdef/`
...
[source]
----
corevance-e2e-tests-runner/build/cucumber-reports/
corevance-e2e-tests-runner/build/reports/tests/
----
