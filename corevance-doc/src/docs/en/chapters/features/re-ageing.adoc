...
This document specifies the functional and business requirements for the Re-Aging feature for Progressive loan products in Corevance.
...
* Non-Progressive loan strategies * Charge-off loan accounts * Interest-bearing loans outside the scope of Corevance Progressive product definition * External API documentation (covered separately)
...
Implementation core is located in PhasedSettlementScheduleProcessor.
...
This means Corevance now supports re-aging before maturity date.
...
Implementation is found in PhasedSettlementScheduleProcessor Entry point for non-interest-bearing loans is `handleReAgeWithCommonStrategy`.

This implementation also contains solution for interest-bearing but not InstalmentCalculator compatible loan schedules.
...
Entrypoint: `PhasedSettlementScheduleProcessor:handleReAgeEqualAmortizationInstalmentCalculator`
...
* handle charges
* collect paidInAdvanceBalances - to move them into the special installment if required
* update model with InstalmentCalculator - see corresponding steps in next chapter
