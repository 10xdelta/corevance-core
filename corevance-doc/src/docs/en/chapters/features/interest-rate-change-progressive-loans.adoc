...
:description: Developer documentation for the interest rate modification feature in Apache Corevance
:keywords: corevance, loans, interest rate, progressive loans, EMI, API, developer
...
The core concept is that the `PhasedSettlementScheduleProcessor` processes transactions in order of their effective date, allowing it to handle backdated transaction cases.

The transaction processor uses the `InstalmentCalculator` to manage interest rate changes over time, ensuring that changes only affect future transactions relative to the actual processing transaction. The `ProgressiveLoanInterestScheduleModel` is responsible for holding and calculating interest for future installments.
