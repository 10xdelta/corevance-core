...
Contract Termination in Apache Corevance is a loan management feature that allows financial institutions to terminate loan contracts. When applied to loans with unpaid installments, this functionality accelerates the maturity date and makes the outstanding loan balance immediately due as of the termination date.
...
The Contract Termination transaction in Apache Corevance performs the following actions:
