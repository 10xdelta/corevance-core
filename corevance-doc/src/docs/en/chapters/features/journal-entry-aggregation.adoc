...
== Overview
The Journal Entry Aggregation Job is a Spring Batch-based solution designed to efficiently aggregate journal entries in the Corevance system. This job processes journal entries in configurable chunks, improving performance and resource utilization when dealing with large volumes of financial transactions.
...
corevance.job.journal-entry-aggregation.exclude-recent-N-days=1

# Chunk size for batch processing
corevance.job.journal-entry-aggregation.chunk-size=1000
----

== Usage

=== Manual Execution
Trigger the job manually through the Corevance API:
