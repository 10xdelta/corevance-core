...
We've configured a global attribute called `\{rootdir}` that you can use to reference these files consistently from Corevance's project root folder.
...
For example, if we wanted to highlight a specific section of the following Cucumber test definition (more on that in section Cucumber Testing) `ClasspathDuplicatesStepDefinitions.java` file located under `corevance-provider/src/test/java/com/corevance/infrastructure/classpath/`.

[source,asciidoc]
--
[source,java,indent=0]
----
\include::{rootdir}/corevance-provider/src/test/java/com/corevance/infrastructure/classpath/ClasspathDuplicatesStepDefinitions.java[tag=then]
----
--
...
[source,asciidoc]
----
http://corevance.apache.org/[Corevance Website]
----
...
[source,asciidoc]
----
For more about upgrades, see xref:release:process.adoc[Corevance Release Process].
----
