...
Corevance provides extension points to define custom batch jobs using module system. Using this approach custom batch jobs can be defined and configured along with Corevance's default batch jobs to extend or customize batch processing.
...
2. Create job configuration to register job, job steps, tasklet with job builder factory. (e. g. `com.acme.corevance.loan.job.AcmeNoopJobConfiguration`)
3. Create tasklet for job execution functionality. (e.g. `com.acme.corevance.loan.job.AcmeNoopJobTasklet`)
...
5. New job name should be registered along with default jobs so that it can be scheduled at startup. For registering job name with Corevance job scheduler, create an enum with job name details (e.g. `com.acme.corevance.loan.job.AcmeJobName`) and a job name provider configuration which is accessed by Corevance job scheduler at startup to retrieve job name (e.g. `com.acme.corevance.loan.job.AcmeJobNameConfig`).
...
[source,java]
----
include::{rootdir}/custom/acme/loan/job/src/main/java/com/acme/corevance/loan/job/AcmeNoopJobConfiguration.java[lines=19..]
----
...
[source,java]
----
include::{rootdir}/custom/acme/loan/job/src/main/java/com/acme/corevance/loan/job/AcmeNoopJobTasklet.java[lines=19..]
----
...
[source,java]
----
include::{rootdir}/custom/acme/loan/job/src/main/java/com/acme/corevance/loan/job/AcmeJobName.java[lines=19..]
----

.Job Name Provider Configuration Example
[source,java]
----
include::{rootdir}/custom/acme/loan/job/src/main/java/com/acme/corevance/loan/job/AcmeJobNameConfig.java[lines=19..]
----
