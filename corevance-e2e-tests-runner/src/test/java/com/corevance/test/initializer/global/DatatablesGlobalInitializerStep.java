...
public class DatatablesGlobalInitializerStep implements CorevanceGlobalInitializerStep {
    ...
    private void createDatatableIdempotent(PostDataTablesRequest datatableRequest) {
        ...
        try {
            corevanceClient.dataTables().getDatatable(datatableName, Map.of());
        } catch (Exception e) {
            log.debug("Datatable '{}' does not exist yet, will create it", datatableName);
            executeVoid(() -> corevanceClient.dataTables().createDatatable(datatableRequest, Map.of()));
        }
    }
}
