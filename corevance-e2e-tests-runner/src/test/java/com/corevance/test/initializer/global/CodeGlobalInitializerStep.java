...
import com.corevance.client.models.PostCodesRequest;
import com.corevance.client.models.PutCodeValuesDataRequest;
...
public class CodeGlobalInitializerStep implements CorevanceGlobalInitializerStep {
    ...
    private void fetchExistingCodesAndCodeValues() {
        List<GetCodesResponse> existingCodes = corevanceClient.codes().retrieveAllCodes();
        existingCodes.forEach(code -> {
            List<GetCodeValuesDataResponse> existingCodeValues = corevanceClient.codeValues()
    ...
    public void createCodeValues(String codeName, List<String> codeValueNames) {
        ...
        codeValueNames.forEach(codeValueName -> {
            ...
            try {
                executeVoid(() -> corevanceClient.codeValues().createCodeValueByCodeName(codeName, postCodeValuesDataRequest, Map.of()));
    ...
    public void updateCodeValues(String codeName, List<String> codeValueNames) {
        codeValueNames.forEach(name -> {
            ...
            executeVoid(() -> corevanceClient.codeValues().updateCodeValueByCodeName(codeName, (long) position, putCodeValuesDataRequest,
                    Map.of()));
        });
    }
        ...
        List.of(CodeNames.FINANCIAL_INSTRUMENT.getValue(), CodeNames.TRANSACTION_TYPE.getValue(), CodeNames.BANKRUPTCY_TAG.getValue(),
                ...
                .forEach(codeName -> {
                    if (existingCodeAndCodeValues.get(codeName) == null) {
                        executeVoid(() -> corevanceClient.codes().createCode(new PostCodesRequest().name(codeName), Map.of()));
                    } else {
