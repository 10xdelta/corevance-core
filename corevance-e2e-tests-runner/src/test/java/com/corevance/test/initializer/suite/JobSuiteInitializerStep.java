 ...
 * under the License.
 */
package com.corevance.test.initializer.suite;
...
@Slf4j
@Component
public class JobSuiteInitializerStep implements CorevanceSuiteInitializerStep {
    ...
    public JobSuiteInitializerStep(CorevanceFeignClient corevanceClient) {
        log.debug("=== JobSuiteInitializerStep: Constructor called - bean is being created ===");
        this.corevanceClient = corevanceClient;
        log.debug("=== JobSuiteInitializerStep: CorevanceFeignClient injected successfully ===");
    }
    ...
    private void enableAndExecuteEventJob() throws InterruptedException {
        ...
        executeVoid(() -> corevanceClient.scheduler().handleCommandsScheduler("start", Map.of()));
        ...
        executeVoid(() -> corevanceClient.schedulerJob().executeJob(jobId, new ExecuteJobRequest(), Map.of("command", "executeJob")));
    ...
    private Long getJobRunCount(Long jobId) {
        try {
            var history = ok(() -> corevanceClient.schedulerJob().retrieveHistory(jobId, Map.of()));
    ...
    public void resetAfterSuite() {
        ...
        try {
            executeVoid(() -> corevanceClient.scheduler().handleCommandsScheduler(Map.of("command", "stop")));
    ...
    private Long updateExternalEventJobFrequency(String cronExpression) {
        GetJobsResponse externalEventJobResponse = ok(() -> corevanceClient.schedulerJob().retrieveAllSchedulerJobs()).stream()
        ...
        executeVoid(() -> corevanceClient.schedulerJob().updateJobDetail(jobId, new PutJobsJobIDRequest().cronExpression(cronExpression)));
        return jobId;
    }
}
