 ...
 * under the License.
 */
package com.corevance.cob.listener;
...
public class COBExecutionListenerRunner implements JobExecutionListener {
    ...
    private final List<CorevanceCOBBeforeJobListener> beforeJobListeners = new ArrayList<>();
    private final List<CorevanceCOBAfterJobListener> afterJobListeners = new ArrayList<>();
    ...
    private void addAfterListeners(ApplicationContext applicationContext, String jobName) {
        List<String> afterListenerClassNames = Arrays.stream(applicationContext.getBeanNamesForType(CorevanceCOBAfterJobListener.class))
                .toList();
        for (String afterListenerClassName : afterListenerClassNames) {
            CorevanceCOBAfterJobListener afterListener = (CorevanceCOBAfterJobListener) applicationContext.getBean(afterListenerClassName);
    ...
    private void addBeforeListeners(ApplicationContext applicationContext, String jobName) {
        List<String> beforeListenerClassNames = Arrays.stream(applicationContext.getBeanNamesForType(CorevanceCOBBeforeJobListener.class))
                .toList();
        for (String beforeListenerClassName : beforeListenerClassNames) {
            CorevanceCOBBeforeJobListener beforeListener = (CorevanceCOBBeforeJobListener) applicationContext
