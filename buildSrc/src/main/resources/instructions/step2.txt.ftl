...
"project = COREVANCE and resolution = fixed and fixVersion is empty"

Move all unresolved JIRA issues which have this release as Fix Version to the next release

"project = COREVANCE and fixVersion = ${project['corevance.release.version']} and status not in ( Resolved, Done, Accepted, Closed )"

You can also run the following query to make sure that the issues fixed for the to-be-released version look accurate:

"project = COREVANCE and fixVersion = ${project['corevance.release.version']}"
