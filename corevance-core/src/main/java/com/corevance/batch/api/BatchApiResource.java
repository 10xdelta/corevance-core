 ...
 * under the License.
 */
package com.corevance.batch.api;
...
import com.corevance.batch.service.BatchApiService;
import com.corevance.infrastructure.core.config.CorevanceProperties;
import com.corevance.infrastructure.security.exception.InvalidInstanceTypeMethodException;
 ...
 * {@link com.corevance.batch.service.BatchApiService} and de-serializes the incoming JSON string to a list of
 * {@link com.corevance.batch.domain .BatchRequest} type. This list is forwarded to BatchApiService which finally
 * returns a list of {@link com.corevance.batch.domain.BatchResponse} type which is then serialized into JSON
...
@Tag(name = "Batch API", description = """
        The Apache Corevance Batch API enables a consumer to access significant amounts of data in a single call or to make changes to several objects at once. Batching allows a consumer to pass instructions for several operations in a single HTTP request. A consumer can also specify dependencies between related operations. Once all operations have been completed, a consolidated response will be passed back and the HTTP connection will be closed.
...
public class BatchApiResource {
    ...
    private void validateRequestMethodsAllowedOnInstanceType(final List<BatchRequest> requestList) {
        ...
        if (corevanceProperties.getMode().isReadOnlyMode()) {
