 ...
 * under the License.
 */
package com.corevance.commands.configuration;
...
import com.corevance.batch.service.BatchExecutionException;
...
public class RetryConfigurationAssembler {
    ...
    private void setLastException(Object ex) {
        corevanceRequestContextHolder.setAttribute(LAST_EXECUTION_EXCEPTION_KEY, ex.getClass());
    }

    public Class<?> getLastException() {
        return (Class<?>) corevanceRequestContextHolder.getAttribute(RetryConfigurationAssembler.LAST_EXECUTION_EXCEPTION_KEY);
    }

    public Retry getRetryConfigurationForExecuteCommand() {
        Class<? extends Throwable>[] exceptionList = corevanceProperties.getRetry().getInstances().getExecuteCommand().getRetryExceptions();
    ...
    private RetryConfig.Builder<Throwable> buildCommonExecuteCommandConfiguration() {
        var props = corevanceProperties.getRetry().getInstances().getExecuteCommand();
