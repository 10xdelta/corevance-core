 ...
 * under the License.
 */
package com.corevance.organisation.monetary.domain;
...
public interface ApplicationCurrencyRepository
    ...
    String FIND_CURRENCY_DETAILS = "SELECT new com.corevance.organisation.monetary.data.CurrencyData(ac.code, ac.name, ac.decimalPlaces, ac.inMultiplesOf, ac.displaySymbol, ac.nameCode) FROM ApplicationCurrency ac ";

    @Cacheable(key = "T(com.corevance.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat('entity_' + #currencyCode)")
    ApplicationCurrency findOneByCode(String currencyCode);

    @Cacheable(key = "T(com.corevance.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat('data_' + #currencyCode)")
    @Query(FIND_CURRENCY_DETAILS + " WHERE ac.code = :code")
    CurrencyData findCurrencyDataByCode(@Param("code") String currencyCode);

    @Cacheable(key = "T(com.corevance.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat('allSorted_' + #sort)")
