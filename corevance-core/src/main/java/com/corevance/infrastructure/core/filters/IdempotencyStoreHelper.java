...
import com.corevance.commands.service.CommandSourceService;
...
public class IdempotencyStoreHelper {
    ...
    public boolean isStoreIdempotencyKey(HttpServletRequest request) {
        return Optional
                .ofNullable(
                        corevanceRequestContextHolder.getAttribute(SynchronousCommandProcessingService.IDEMPOTENCY_KEY_STORE_FLAG, request))
    ...
    public Optional<Long> getCommandId(HttpServletRequest request) {
        return Optional
                .ofNullable(corevanceRequestContextHolder.getAttribute(SynchronousCommandProcessingService.COMMAND_SOURCE_ID, request))
