...
import com.corevance.infrastructure.security.service.PasswordEncryptor;
...
public class DatabasePasswordEncryptor implements PasswordEncryptor {
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            System.out.println(
                    "Usage: java -cp corevance-provider.jar -Dloader.main=com.corevance.infrastructure.core.service.database.DatabasePasswordEncryptor org.springframework.boot.loader.launch.PropertiesLauncher <masterPassword> <plainPassword>");
            System.exit(1);
        }
    ...
    public String encrypt(String plainPassword) {
        String masterPassword = Optional.ofNullable(corevanceProperties.getTenant())
                .map(CorevanceProperties.CorevanceTenantProperties::getMasterPassword)
                .orElse(corevanceProperties.getDatabase().getDefaultMasterPassword());
        String encryption = Optional.ofNullable(corevanceProperties.getTenant())
                .map(CorevanceProperties.CorevanceTenantProperties::getEncryption).orElse(DEFAULT_ENCRYPTION);
    ...
    public String getMasterPasswordHash() {
        String masterPassword = Optional.ofNullable(corevanceProperties) //
                .map(CorevanceProperties::getTenant) //
                .map(CorevanceProperties.CorevanceTenantProperties::getMasterPassword) //
                .orElse(corevanceProperties.getDatabase().getDefaultMasterPassword());
