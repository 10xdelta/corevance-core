...
@Slf4j
public class CorevanceRemoteJobMessageHandlerCondition extends PropertiesCondition {

    @Override
    protected boolean matches(CorevanceProperties properties) {
    ...
    private boolean isOnlyOneMessageHandlerEnabled(CorevanceProperties properties) {
        boolean isSpringEventsEnabled = properties.getRemoteJobMessageHandler().getSpringEvents().isEnabled();
        boolean isJmsEnabled = properties.getRemoteJobMessageHandler().getJms().isEnabled();
        return isSpringEventsEnabled ^ isJmsEnabled;
    }

    private boolean isAnyMessageHandlerConfigured(CorevanceProperties properties) {
        boolean isSpringEventsEnabled = properties.getRemoteJobMessageHandler().getSpringEvents().isEnabled();
        boolean isJmsEnabled = properties.getRemoteJobMessageHandler().getJms().isEnabled();
        return isSpringEventsEnabled || isJmsEnabled;
    }

    private boolean isBatchInstance(CorevanceProperties properties) {
    ...
    private boolean isBatchManagerAndWorkerTogether(CorevanceProperties properties) {
        boolean isBatchManagerModeEnabled = properties.getMode().isBatchManagerEnabled();
        boolean isBatchWorkerModeEnabled = properties.getMode().isBatchWorkerEnabled();
        return isBatchManagerModeEnabled && isBatchWorkerModeEnabled;
    }
