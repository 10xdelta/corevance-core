...
public class JdbcTenantDetailsService implements TenantDetailsService {
    ...
    public CorevancePlatformTenant loadTenantById(final String tenantIdentifier) {
    ...
    @Override
    public List<CorevancePlatformTenant> findAllTenants() {
        final TenantMapper rm = new TenantMapper(false);
        final String sql = "select  " + rm.schema();

        final List<CorevancePlatformTenant> corevancePlatformTenants = this.jdbcTemplate.query(sql, rm); // NOSONAR
        return corevancePlatformTenants;
    }
}
