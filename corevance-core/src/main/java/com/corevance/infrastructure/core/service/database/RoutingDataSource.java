...
public class RoutingDataSource extends AbstractDataSource {
    ...
    @Value("${corevance.datasource.connection-checkout-diagnostics.enabled:false}")
    private boolean connectionCheckoutDiagnosticsEnabled;

    @Value("${corevance.datasource.connection-checkout-diagnostics.stack-depth:12}")
    ...
    private String tenant() {
        CorevancePlatformTenant tenant = ThreadLocalContextUtil.getTenant();
        if (tenant == null) {
            return "<none>";
        }
