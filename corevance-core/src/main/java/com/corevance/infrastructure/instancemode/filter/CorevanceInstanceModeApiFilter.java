...
import static com.corevance.infrastructure.instancemode.filter.CorevanceInstanceModeApiFilter.ExceptionListItem.item;
...
import com.corevance.infrastructure.core.data.ApiGlobalErrorResponse;
import org.apache.http.HttpStatus;
import org.springframework.web.filter.OncePerRequestFilter;

@RequiredArgsConstructor
public class CorevanceInstanceModeApiFilter extends OncePerRequestFilter {

    private static final List<ExceptionListItem> EXCEPTION_LIST = List.of(
            item(CorevanceProperties.CorevanceModeProperties::isBatchManagerEnabled, pi -> pi.startsWith("/v1/jobs")),
            item(CorevanceProperties.CorevanceModeProperties::isBatchManagerEnabled, pi -> pi.startsWith("/v1/scheduler")),
            item(CorevanceProperties.CorevanceModeProperties::isBatchManagerEnabled, pi -> pi.startsWith("/v1/loans/catch-up")),
            item(CorevanceProperties.CorevanceModeProperties::isBatchManagerEnabled, pi -> pi.startsWith("/v1/loans/is-catch-up-running")),
    ...
    protected void doFilterInternal(final HttpServletRequest request, final HttpServletResponse response, final FilterChain filterChain)
        ...
        if (isOnExceptionList(request) || isActuatorApi(request)) {
            ...
            if (isPathOnExceptionList(request)) {
                ...
                if (corevanceProperties.getMode().isReadEnabled() && isReadMethod(request)) {
                    proceed(filterChain, request, response);
                } else if (corevanceProperties.getMode().isWriteEnabled()) {
                    proceed(filterChain, request, response);
                } else {
                    reject(request, response);
                }
    ...
    private boolean isOnExceptionList(HttpServletRequest request) {
        ...
        return EXCEPTION_LIST.stream().anyMatch(
                item -> item.getModeFunction().apply(corevanceProperties.getMode()) && item.getPathFunction().apply(request.getPathInfo()));
    }
    ...
    static class ExceptionListItem {
        ...
        private final Function<CorevanceProperties.CorevanceModeProperties, Boolean> modeFunction;
        private final Function<String, Boolean> pathFunction;

        public static ExceptionListItem item(Function<CorevanceProperties.CorevanceModeProperties, Boolean> modeFunction,
