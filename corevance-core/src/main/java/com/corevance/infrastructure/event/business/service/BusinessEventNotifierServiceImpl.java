 ...
 * under the License.
 */
package com.corevance.infrastructure.event.business.service;
...
import com.corevance.infrastructure.event.business.domain.NoExternalEvent;
import com.corevance.infrastructure.event.external.service.ExternalEventService;
...
public class BusinessEventNotifierServiceImpl implements BusinessEventNotifierService, InitializingBean, TransactionExecutionListener {
    ...
    private boolean isExternalEventPostingEnabled() {
        return corevanceProperties.getEvents().getExternal().isEnabled();
    }
    ...
    private void storeTransactionalBusinessEvent(BusinessEvent<?> businessEvent) {
        ...
        CorevanceContext corevanceContext = ThreadLocalContextUtil.getContext();
        businessEvents.add(new BusinessEventWithContext(businessEvent, corevanceContext));
    }
    ...
    public void beforeCommit(@NonNull final TransactionExecution transaction) {
        ...
        final CorevanceContext originalContext = ThreadLocalContextUtil.getContext();
        businessEventWithContexts.forEach(businessEventWithContext -> {
            final CorevanceContext currentContext = businessEventWithContext.getCorevanceContext();
    ...
    private static final class BusinessEventWithContext {
        ...
        private CorevanceContext corevanceContext;

        BusinessEventWithContext(BusinessEvent<?> event, CorevanceContext corevanceContext) {
            this.event = event;
            this.corevanceContext = corevanceContext;
        }
    }
}
