 ...
 * under the License.
 */
package com.corevance.integrationtests.common.charges;
...
public final class ChargesHelper {
    ...
    private static final String CHARGES_URL = "/corevance-provider/api/v1/charges";
    ...
    public PostChargesResponse createCharges(ChargeRequest request) {
        return Calls.ok(CorevanceClientHelper.getCorevanceClient().charges.createCharge(request));
    }

    public GetChargesResponse retrieveCharge(final Long chargeId) {
        return Calls.ok(CorevanceClientHelper.getCorevanceClient().charges.retrieveOneCharge(chargeId));
    }
}
