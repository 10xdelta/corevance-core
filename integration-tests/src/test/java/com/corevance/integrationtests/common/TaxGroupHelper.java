...
public final class TaxGroupHelper {
    ...
    public static PostTaxesGroupResponse createTaxGroup(PostTaxesGroupRequest request) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().taxGroup().createTaxGroup(request));
    }

    public static GetTaxesGroupResponse retrieveTaxGroup(Long taxGroupId) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().taxGroup().retrieveOneTaxGroup(taxGroupId));
    }

    public static List<GetTaxesGroupResponse> retrieveAllTaxGroups() {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().taxGroup().retrieveAllTaxGroups());
    }
}
