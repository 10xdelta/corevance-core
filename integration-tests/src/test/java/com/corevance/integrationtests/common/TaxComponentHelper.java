...
public final class TaxComponentHelper {
    ...
    public static PostTaxesComponentsResponse createTaxComponent(PostTaxesComponentsRequest request) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().taxComponents().createTaxComponent(request));
    }

    public static GetTaxesComponentsResponse retrieveTaxComponent(Long taxComponentId) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().taxComponents().retrieveOneTaxComponent(taxComponentId));
    }
}
