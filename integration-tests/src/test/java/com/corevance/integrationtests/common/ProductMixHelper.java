...
public class ProductMixHelper {
    ...
    public List<GetLoanProductsResponse> getProductsMixList() {
        ...
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().loanProducts().retrieveAllLoanProducts());
    }

    public GetLoanProductsTemplateResponse getProductMixTemplate() {
        log.info("-------------------- RETRIEVING PRODUCT MIX TEMPLATE ---------------------");
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().loanProducts().retrieveTemplateLoanProduct(true));
    }
}
