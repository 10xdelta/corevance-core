...
import com.corevance.client.models.QuestionData;
import com.corevance.client.models.ResponseData;
import com.corevance.client.models.SurveyData;
...
public class SurveyHelper {
    ...
    public Long createSurvey(String name, String description, LocalDate validFrom, LocalDate validTo, List<String> questions) {
        ...
        executeVoid(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().spmSurveys().createSurvey(surveyData));
    ...
    public SurveyData retrieveSurvey(Long surveyId) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().spmSurveys().findSurvey(surveyId));
    }

    public List<SurveyData> retrieveAllSurveys() {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().spmSurveys().fetchAllSurveys((Boolean) null));
    }

    public List<SurveyData> retrieveActiveSurveys() {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().spmSurveys().fetchAllSurveys(true));
    }

    public String updateSurvey(Long surveyId, SurveyData surveyData) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().spmSurveys().editSurvey(surveyId, surveyData));
    }

    public void deactivateSurvey(Long surveyId) {
        executeVoid(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().spmSurveys().activateOrDeactivateSurvey(surveyId,
                DEACTIVATE_COMMAND));
