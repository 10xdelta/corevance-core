...
import static com.corevance.infrastructure.jobs.api.SchedulerJobApiConstants.SHORT_NAME_PARAM;
...
import com.corevance.client.models.PutJobsJobIDRequest;
...
public class SchedulerJobHelper {
    ...
    private List<Map<String, Object>> getAllSchedulerJobs() {
        final String GET_ALL_SCHEDULER_JOBS_URL = "/corevance-provider/api/v1/jobs?" + Utils.TENANT_IDENTIFIER;
    ...
    public Map<String, Object> getSchedulerJobById(int jobId) {
        final String GET_SCHEDULER_JOB_BY_ID_URL = "/corevance-provider/api/v1/jobs/" + jobId + "?" + Utils.TENANT_IDENTIFIER;
    ...
    public Boolean getSchedulerStatus() {
        final String GET_SCHEDULER_STATUS_URL = "/corevance-provider/api/v1/scheduler?" + Utils.TENANT_IDENTIFIER;
    ...
    public void updateSchedulerStatus(final boolean on) {
        ...
        Calls.ok(CorevanceClientHelper.getCorevanceClient().jobsScheduler.handleCommandsScheduler(command));
    }
    ...
    public Map<String, Object> updateSchedulerJob(int jobId, final boolean active) {
        final String UPDATE_SCHEDULER_JOB_URL = "/corevance-provider/api/v1/jobs/" + jobId + "?" + Utils.TENANT_IDENTIFIER;
    ...
    public void updateSchedulerJob(long jobId, PutJobsJobIDRequest request) {
        Calls.ok(CorevanceClientHelper.getCorevanceClient().jobs.updateJobDetail(jobId, request));
    }
    ...
    public void runSchedulerJob(int jobId) {
        Calls.ok(CorevanceClientHelper.getCorevanceClient().jobs.executeJob((long) jobId, "executeJob"));
    }
    ...
    public void runSchedulerJob(int jobId, ResponseSpecification responseSpec) {
        final String RUN_SCHEDULER_JOB_URL = "/corevance-provider/api/v1/jobs/" + jobId + "?command=executeJob&" + Utils.TENANT_IDENTIFIER;
    ...
    public void runSchedulerJobByShortName(String shortName, ResponseSpecification responseSpec) {
        final String RUN_SCHEDULER_JOB_URL = "/corevance-provider/api/v1/jobs/" + SHORT_NAME_PARAM + "/" + shortName + "?command=executeJob&"
    ...
    public int getSchedulerJobIdByName(String jobName) {
        ...
        throw new IllegalArgumentException(
                "No such named Job (see com.corevance.infrastructure.jobs.service.JobName enum):" + jobName);
    }

    public Long getSchedulerJobIdByShortName(String shortName) {
        LOG.info("------------------------ RETRIEVING SCHEDULER JOB ID BY SHORT NAME -------------------------");
        GetJobsResponse job = Calls.ok(CorevanceClientHelper.getCorevanceClient().jobs.retrieveByShortName(shortName));
        assertNotNull(job);
        return job.getJobId();
    }
     ...
     *
     * @param jobName
     *            displayName (see {@link com.corevance.infrastructure.jobs.service.JobName}) of Scheduler Job
     *
    ...
    private Map<String, Object> getLatestJobRunHistory(long jobId) {
        final String GET_LATEST_SCHEDULER_JOB_RUN_HISTORY_URL = "/corevance-provider/api/v1/jobs/" + jobId
