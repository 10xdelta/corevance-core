...
import com.corevance.client.models.PostHolidaysRequest;
import com.corevance.client.models.PostHolidaysRequestOffices;
...
public final class HolidayHelper {
    ...
    public static Long createHolidays() {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().holidays().createHoliday(getCreateHolidayRequest()))
                .getResourceId();
    }

    public static Long createTyoe1Holidays() {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().holidays().createHoliday(getCreateType1HolidayRequest()))
                .getResourceId();
    }

    public static Long activateHolidays(final Long holidayId) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().holidays().handleCommandsHoliday(holidayId,
                Collections.emptyMap(), "activate")).getResourceId();
    }

    public static GetHolidaysResponse getHolidayById(final Long holidayId) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().holidays().retrieveOneHoliday(holidayId));
    }

    public static Long deleteHoliday(final Long holidayId) {
        return ok(() -> CorevanceFeignClientHelper.getCorevanceFeignClient().holidays().deleteHoliday(holidayId)).getResourceId();
    }
