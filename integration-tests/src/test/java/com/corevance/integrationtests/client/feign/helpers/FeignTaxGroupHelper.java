...
public class FeignTaxGroupHelper {
    ...
    public FeignTaxGroupHelper(CorevanceFeignClient corevanceClient) {
        this.corevanceClient = corevanceClient;
    }

    public PostTaxesGroupResponse createTaxGroup(PostTaxesGroupRequest request) {
        return ok(() -> corevanceClient.taxGroup().createTaxGroup(request));
    }

    public GetTaxesGroupResponse retrieveTaxGroup(Long taxGroupId) {
        return ok(() -> corevanceClient.taxGroup().retrieveOneTaxGroup(taxGroupId));
    }

    public List<GetTaxesGroupResponse> retrieveAllTaxGroups() {
        return ok(() -> corevanceClient.taxGroup().retrieveAllTaxGroups());
    }

    public CallFailedRuntimeException retrieveTaxGroupExpectingError(Long taxGroupId) {
        return fail(() -> corevanceClient.taxGroup().retrieveOneTaxGroup(taxGroupId));
    }
}
