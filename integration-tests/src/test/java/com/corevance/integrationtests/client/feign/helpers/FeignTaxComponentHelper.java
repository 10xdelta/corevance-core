...
public class FeignTaxComponentHelper {
    ...
    public FeignTaxComponentHelper(CorevanceFeignClient corevanceClient) {
    ...
    public PostTaxesComponentsResponse createTaxComponent(PostTaxesComponentsRequest request) {
        return ok(() -> corevanceClient.taxComponents().createTaxComponent(request));
    }

    public GetTaxesComponentsResponse retrieveTaxComponent(Long taxComponentId) {
        return ok(() -> corevanceClient.taxComponents().retrieveOneTaxComponent(taxComponentId));
    }

    public CallFailedRuntimeException retrieveTaxComponentExpectingError(Long taxComponentId) {
        return fail(() -> corevanceClient.taxComponents().retrieveOneTaxComponent(taxComponentId));
    }
}
