...
class StaffTest extends IntegrationTest {
    ...
    Long create() {
        return ok(corevanceClient().staff.createStaff(new StaffCreateRequest().officeId(1L).firstname(Utils.randomFirstNameGenerator())
    ...
    Optional<Long> retrieveFirst() {
        var staff = ok(corevanceClient().staff.retrieveAllStaff(1L, true, false, "ACTIVE"));
        if (!staff.isEmpty()) {
