...
public class OfficeTest extends IntegrationTest {
    ...
    void createOne() {
        ...
        // TODO requiring dateFormat(..).locale(..) is dumb :( see https://issues.apache.org/jira/browse/COREVANCE-1233
        assertThat(ok(corevanceClient().offices.createOffice(new PostOfficesRequest().name(Utils.randomStringGenerator("TestOffice_", 6))
    ...
    @Test
    @Order(2)
    void retrieveOneExistingInclDateFormat() { // see COREVANCE-1220 re. what this tests re. Date Format
        List<GetOfficesResponse> response = ok(corevanceClient().offices.retrieveAllOffices(true, null, null));
    ...
    // defined in application.properties (corevance.input-validation.patterns[4],
    ...
    void retrieveAllOffices_withWhitelistedOrderByColumn_succeeds(String column) throws Exception {
        var response = corevanceClient().offices.retrieveAllOffices(false, column, null).execute();
    ...
    void retrieveAllOffices_withBlankOrderBy_isAcceptedAsAbsent(String blank) throws Exception {
        var response = corevanceClient().offices.retrieveAllOffices(false, blank, null).execute();
    ...
    void retrieveAllOffices_zdres035SubqueryPayloads_areRejected(String payload) throws Exception {
        var response = corevanceClient().offices.retrieveAllOffices(false, payload, null).execute();
    ...
    void retrieveAllOffices_sleepSubquery_isRejectedBeforeDatabaseExecution() throws Exception {
        ...
        var response = corevanceClient().offices.retrieveAllOffices(false, "(SELECT SLEEP(3))", null).execute();
