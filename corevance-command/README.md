...
Corevance accumulated some technical debt over the years. One area that is implicated is type-safety of internal and external facing APIs, the most prominent of which is Corevance's REST API. In general the package layout of the project reflects a more or less classic layered architecture (REST API, data transfer/value objects, business logic services, storage/repositories). The project predates some of the more modern frameworks and best practices that are available today and on occasions the data structures that are exchanged offer some challenges (e.g. generic types). Corevance's code base reflects that, especially where JSON de-/serialization is involved. Nowadays, this task would be simply delegated to the Jackson framework, but when Corevance (Mifos) started the decision was made to use Google's GSON library and create handcrafted helper classes to deal with JSON parsing. While this provided a lot of flexibility this approach had some downsides:
...
- now the business logic is unnecessarily aware of how Corevance communicates to the outside world and makes replacing/enhancing the communication protocol (e.g. with GRPC) pretty much impossible
...
There has been already some preparatory work done concerning type safety, but until now we avoided dealing with the real source of this issue. Corevance's architectures devises read from write requests ("CQRS", https://martinfowler.com/bliki/CQRS.html) for improved scalability.
...
```
git clone git@github.com:vidakovic/corevance.git
cd corevance
git switch feature/COREVANCE-2169
./gradlew :corevance-command:build
./gradlew :corevance-command:jmh
```
...
- https://issues.apache.org/jira/browse/COREVANCE-2169
- https://issues.apache.org/jira/browse/COREVANCE-2021
- https://issues.apache.org/jira/browse/COREVANCE-1744
- https://issues.apache.org/jira/browse/COREVANCE-1909
